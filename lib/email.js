...
export function proposalMessageEmail({ recipientName, senderRole, campaignTitle, messagePreview, proposalId }) {
  const body = `
    <h1 style="margin:0 0 8px;font-size:24px;font-weight:700;color:#111827;">💬 New Message</h1>
    <p style="margin:0 0 8px;font-size:15px;color:#6b7280;line-height:1.6;">
      Hi ${recipientName}, you have a new message on the proposal for <strong>"${campaignTitle}"</strong> from the ${senderRole}.
    </p>
    <div style="margin:0 0 24px;background:#f9fafb;border-left:3px solid ${BRAND};padding:12px 16px;border-radius:0 8px 8px 0;font-size:14px;color:#374151;line-height:1.6;">
      "${messagePreview}"
    </div>
    <a href="https://erasekit.vercel.app/proposals/${proposalId}"
       style="display:inline-block;background:${BRAND};color:#ffffff;text-decoration:none;font-size:14px;font-weight:600;padding:12px 28px;border-radius:8px;">
  ...
  return {
    from: FROM,
    to: undefined,
    subject: `New message on proposal: "${campaignTitle}"`,
    html: baseTemplate(body),
  }
}

export function applicationAcceptedEmail({ influencerName, campaignTitle, workspaceUrl }) {
  const body = `
    <h1 style="margin:0 0 8px;font-size:24px;font-weight:700;color:#111827;">🎉 Application Accepted!</h1>
    <p style="margin:0 0 8px;font-size:15px;color:#6b7280;line-height:1.6;">
      Congratulations ${influencerName}! Your application for <strong>"${campaignTitle}"</strong> has been accepted by the brand.
    </p>
    <p style="margin:0 0 24px;font-size:14px;color:#6b7280;">Your campaign workspace is ready. Click below to get started:</p>
    <a href="${workspaceUrl}"
       style="display:inline-block;background:${BRAND};color:#ffffff;text-decoration:none;font-size:14px;font-weight:600;padding:12px 28px;border-radius:8px;">
      Open Campaign Workspace →
    </a>
    <p style="margin:24px 0 8px;font-size:14px;font-weight:600;color:#374151;">In your workspace you can:</p>
    <ul style="margin:0 0 24px;padding-left:20px;font-size:14px;color:#6b7280;line-height:2;">
      <li>Track campaign milestones</li>
      <li>Submit content deliverables</li>
      <li>Chat with the brand</li>
      <li>Confirm payment</li>
    </ul>
    <p style="margin:0;font-size:14px;color:#6b7280;">Good luck with the campaign!</p>
  `
  return {
    from: FROM,
    to: undefined,
    subject: `🎉 Your application for "${campaignTitle}" was accepted!`,
    html: baseTemplate(body),
  }
}

export function applicationRejectedEmail({ influencerName, campaignTitle }) {
  const body = `
    <h1 style="margin:0 0 8px;font-size:24px;font-weight:700;color:#111827;">Application Update</h1>
    <p style="margin:0 0 8px;font-size:15px;color:#6b7280;line-height:1.6;">
      Hi ${influencerName}, thank you for applying to <strong>"${campaignTitle}"</strong>.
    </p>
    <p style="margin:0 0 24px;font-size:14px;color:#6b7280;">
      Unfortunately, the brand has decided to move forward with other creators this time.
      Don't be discouraged — keep applying to campaigns that match your niche!
    </p>
    <a href="${process.env.NEXTAUTH_URL}/campaigns"
       style="display:inline-block;background:${BRAND};color:#ffffff;text-decoration:none;font-size:14px;font-weight:600;padding:12px 28px;border-radius:8px;">
      Browse More Campaigns →
    </a>
  `
  return {
    from: FROM,
    to: undefined,
    subject: `Application update for "${campaignTitle}"`,
    html: baseTemplate(body),
  }
}
