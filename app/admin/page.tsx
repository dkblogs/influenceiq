        ...
        <div className="flex gap-2 mb-6 border-b border-gray-100">
          {["overview", "users", "influencers", "campaigns", "transactions"].map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 text-sm font-medium capitalize border-b-2 mb-[-1px] transition-colors ${activeTab === tab ? "border-purple-600 text-purple-600" : "border-transparent text-gray-500 hover:text-gray-900"}`}>
              {tab}
            </button>
          ))}
        </div>
              ...
              <div className="space-y-3">
                {users.slice(0, 5).map((u: any) => (
                  <div key={u.id} className="flex items-center justify-between">
                    ...
                    <div className="text-sm font-medium text-purple-600">{u.credits} cr</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              <tbody>
                {users.map((u: any, i: number) => (
                  <tr key={u.id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    ...
                    <td className="px-6 py-3 font-medium text-purple-600">{u.credits}</td>
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-x-auto">
            <table className="w-full text-sm min-w-[600px]">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="text-left px-6 py-3 font-medium text-gray-500">Name</th>
                  <th className="text-left px-6 py-3 font-medium text-gray-500">Niche</th>
              ...
              <tbody>
                {influencers.map((inf: any, i: number) => (
                  <tr key={inf.id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    ...
                    <td className="px-6 py-3 text-gray-500">{inf.niche}</td>
                    <td className="px-6 py-3 text-gray-500">{inf.platform}</td>
                    <td className="px-6 py-3 font-medium text-purple-600">{inf.score}</td>
                    <td className="px-6 py-3">
                      <button
            ...
            <table className="w-full text-sm min-w-[600px]">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th className="text-left px-6 py-3 font-medium text-gray-500">Niche</th>
                  <th className="text-left px-6 py-3 font-medium text-gray-500">Budget</th>
                  <th className="text-left px-6 py-3 font-medium text-gray-500">Slots</th>
                  <th className="text-left px-6 py-3 font-medium text-gray-500">Status</th>
                  <th className="text-left px-6 py-3 font-medium text-gray-500">Posted</th>
                </tr>
              </thead>
              <tbody>
                {campaigns.map((c: any, i: number) => (
                  <tr key={c.id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    ...
                    <td className="px-6 py-3 text-gray-500">{c.niche}</td>
                    <td className="px-6 py-3 text-gray-500">{c.budget}</td>
                    <td className="px-6 py-3 text-gray-500">{c.slots}</td>
                    <td className="px-6 py-3">
                      <span className="text-xs bg-green-50 text-green-600 px-2 py-0.5 rounded-full">{c.status}</span>
                    </td>
                    <td className="px-6 py-3 text-gray-400">{new Date(c.createdAt).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                {transactions.map((t: any, i: number) => (
                  <tr key={t.id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    ...
                    <td className={`px-6 py-3 font-medium ${t.amount > 0 ? "text-green-600" : "text-red-500"}`}>
