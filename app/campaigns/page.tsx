"use client"
import { useState } from "react"

const allCampaigns = [
  { id: 1, brand: "FreshKart", brandInitials: "FK", brandColor: "bg-orange-500", title: "Summer grocery launch campaign", description: "Looking for food and lifestyle influencers to promote our new summer grocery collection. Must create 2 Instagram reels and 3 stories.", niche: "Food", platform: "Instagram", budget: "₹15,000", deadline: "15 days left", applicants: 12, slots: 3, location: "Pan India", minFollowers: "10K", status: "Open" },
  { id: 2, brand: "ZenFit", brandInitials: "ZF", brandColor: "bg-green-500", title: "App launch — fitness influencers needed", description: "Promote our new AI fitness app to your audience. Create 1 YouTube video and 2 Instagram posts showing the app in action.", niche: "Fitness", platform: "YouTube + Instagram", budget: "₹25,000", deadline: "20 days left", applicants: 8, slots: 2, location: "Mumbai, Delhi, Bangalore", minFollowers: "25K", status: "Open" },
  { id: 3, brand: "PayEasy", brandInitials: "PE", brandColor: "bg-blue-500", title: "Finance creator collab — fintech app", description: "Seeking finance and tech creators to explain our payment solution to small business owners. LinkedIn articles or YouTube explainer videos preferred.", niche: "Finance", platform: "LinkedIn + YouTube", budget: "₹40,000", deadline: "30 days left", applicants: 5, slots: 4, location: "Pan India", minFollowers: "15K", status: "Open" },
  { id: 4, brand: "StyleHub", brandInitials: "SH", brandColor: "bg-pink-500", title: "Festive collection showcase", description: "We need fashion influencers to showcase our new festive ethnic wear collection. 3 Instagram reels required. Products will be provided.", niche: "Fashion", platform: "Instagram", budget: "₹20,000", deadline: "10 days left", applicants: 24, slots: 5, location: "Mumbai, Delhi", minFollowers: "20K", status: "Open" },
  { id: 5, brand: "EduLearn", brandInitials: "EL", brandColor: "bg-yellow-500", title: "Study tips YouTube collab", description: "Partner with us to create study tips and exam prep content. Ideal for education creators with student audiences. Full creative freedom.", niche: "Education", platform: "YouTube", budget: "₹12,000", deadline: "25 days left", applicants: 7, slots: 3, location: "Pan India", minFollowers: "5K", status: "Open" },
  { id: 6, brand: "FoodBox", brandInitials: "FB", brandColor: "bg-red-500", title: "Healthy snack unboxing campaign", description: "Create an unboxing video or reel of our healthy snack subscription box. Honest reviews welcome. Box will be sent to your address.", niche: "Food", platform: "Instagram + YouTube", budget: "₹8,000", deadline: "12 days left", applicants: 18, slots: 6, location: "Pan India", minFollowers: "5K", status: "Open" },
]

const niches = ["All", "Food", "Fitness", "Finance", "Fashion", "Education", "Tech"]
const platforms = ["All", "Instagram", "YouTube", "LinkedIn"]

export default function Campaigns() {
  const [selectedNiche, setSelectedNiche] = useState("All")
  const [selectedPlatform, setSelectedPlatform] = useState("All")
  const [search, setSearch] = useState("")
  const [applied, setApplied] = useState([])

  const filtered = allCampaigns.filter((c) => {
    const matchNiche = selectedNiche === "All" || c.niche === selectedNiche
    const matchPlatform = selectedPlatform === "All" || c.platform.includes(selectedPlatform)
    const matchSearch = c.title.toLowerCase().includes(search.toLowerCase()) ||
      c.brand.toLowerCase().includes(search.toLowerCase()) ||
      c.niche.toLowerCase().includes(search.toLowerCase())
    return matchNiche && matchPlatform && matchSearch
  })

  function handleApply(id) {
    if (!applied.includes(id)) setApplied([...applied, id])
  }

  return (
    <main className="min-h-screen bg-white">

      {/* Navigation */}
      <nav className="flex items-center justify-between px-8 py-4 border-b border-gray-100 sticky top-0 bg-white z-50">
        <a href="/" className="flex items-center gap-2">
          <span className="text-2xl">⚡</span>
          <span className="text-xl font-semibold">Influence<span className="text-purple-600">IQ</span></span>
        </a>
        <div className="flex items-center gap-4">
          <a href="/discover" className="text-sm text-gray-500 hover:text-gray-900">Find Influencers</a>
          <a href="/brands" className="text-sm text-gray-500 hover:text-gray-900">Find Brands</a>
          <a href="/campaigns" className="text-sm text-purple-600 font-medium">Open Campaigns</a>
          <a href="/login" className="text-sm bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700">Get Started</a>
        </div>
      </nav>

      <div className="px-8 py-10 max-w-6xl mx-auto">

        {/* Header */}
        <div className="mb-8">
          <div className="inline-block bg-orange-50 text-orange-700 text-xs px-3 py-1 rounded-full mb-3">For Influencers</div>
          <h1 className="text-3xl font-semibold text-gray-900 mb-1">Open Campaigns</h1>
          <p className="text-gray-500">Brands actively looking for influencers right now. Apply directly — costs 2 credits per application.</p>
        </div>

        {/* Search */}
        <div className="flex gap-3 mb-6">
          <input
            className="flex-1 px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-purple-400"
            placeholder="Search campaigns by title, brand, or niche..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
          {applied.length > 0 && (
            <div className="flex items-center gap-2 px-4 py-2.5 bg-green-50 border border-green-200 rounded-lg text-sm text-green-700">
              ✓ {applied.length} applied
            </div>
          )}
        </div>

        {/* Niche filters */}
        <div className="mb-4">
          <div className="text-xs text-gray-400 mb-2 font-medium uppercase tracking-wide">Niche</div>
          <div className="flex gap-2 flex-wrap">
            {niches.map((n) => (
              <button key={n} onClick={() => setSelectedNiche(n)}
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${selectedNiche === n ? "bg-purple-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                {n}
              </button>
            ))}
          </div>
        </div>

        {/* Platform filters */}
        <div className="mb-8">
          <div className="text-xs text-gray-400 mb-2 font-medium uppercase tracking-wide">Platform</div>
          <div className="flex gap-2">
            {platforms.map((p) => (
              <button key={p} onClick={() => setSelectedPlatform(p)}
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${selectedPlatform === p ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                {p}
              </button>
            ))}
          </div>
        </div>

        {/* Results count */}
        <div className="text-sm text-gray-400 mb-4">
          {filtered.length} open campaign{filtered.length !== 1 ? "s" : ""} found
        </div>

        {/* Campaign list */}
        {filtered.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            <div className="text-4xl mb-3">📋</div>
            <div className="font-medium text-gray-600 mb-1">No campaigns found</div>
            <div className="text-sm">Try a different niche or platform</div>
          </div>
        ) : (
          <div className="space-y-4">
            {filtered.map((c) => (
              <div key={c.id} className="border border-gray-100 rounded-xl p-6 hover:shadow-md transition-all">
                <div className="flex items-start gap-4">

                  {/* Brand avatar */}
                  <div className={`w-12 h-12 rounded-xl ${c.brandColor} flex items-center justify-center text-white font-medium text-sm flex-shrink-0`}>
                    {c.brandInitials}
                  </div>

                  {/* Content */}
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <div className="font-medium text-gray-900 mb-0.5">{c.title}</div>
                        <div className="text-xs text-gray-400">{c.brand} · {c.location}</div>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0 ml-4">
                        <span className="text-xs bg-green-50 text-green-700 px-2 py-0.5 rounded-full font-medium">{c.status}</span>
                        <span className="text-xs text-red-500 font-medium">{c.deadline}</span>
                      </div>
                    </div>

                    <p className="text-sm text-gray-500 mb-4 leading-relaxed">{c.description}</p>

                    <div className="flex items-center gap-4 mb-4 flex-wrap">
                      <div className="flex items-center gap-1.5 text-xs text-gray-500">
                        <span className="text-gray-300">💰</span> {c.budget}
                      </div>
                      <div className="flex items-center gap-1.5 text-xs text-gray-500">
                        <span className="text-gray-300">📱</span> {c.platform}
                      </div>
                      <div className="flex items-center gap-1.5 text-xs text-gray-500">
                        <span className="text-gray-300">👥</span> Min. {c.minFollowers} followers
                      </div>
                      <div className="flex items-center gap-1.5 text-xs text-gray-500">
                        <span className="text-gray-300">🎯</span> {c.slots} slots · {c.applicants} applied
                      </div>
                      <span className="text-xs bg-purple-50 text-purple-700 px-2 py-0.5 rounded-full">{c.niche}</span>
                    </div>

                    {/* Progress bar */}
                    <div className="mb-4">
                      <div className="flex justify-between text-xs text-gray-400 mb-1">
                        <span>{c.applicants} applicants</span>
                        <span>{c.slots} slots available</span>
                      </div>
                      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-purple-400 rounded-full"
                          style={{ width: `${Math.min((c.applicants / (c.slots * 5)) * 100, 100)}%` }}
                        ></div>
                      </div>
                    </div>

                    <div className="flex gap-3">
                      <button
                        onClick={() => handleApply(c.id)}
                        className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${
                          applied.includes(c.id)
                            ? "bg-green-50 text-green-700 border border-green-200"
                            : "bg-purple-600 text-white hover:bg-purple-700"
                        }`}
                      >
                        {applied.includes(c.id) ? "✓ Applied" : "Apply now — 2 credits"}
                      </button>
                      <button className="px-5 py-2 border border-gray-200 text-gray-600 rounded-lg text-sm hover:bg-gray-50 transition-colors">
                        View brand profile
                      </button>
                    </div>
                  </div>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>

      {/* Footer */}
      <footer className="border-t border-gray-100 px-8 py-8 text-center text-sm text-gray-400 mt-8">
        InfluenceIQ · India's AI Influencer Marketplace · 2025
      </footer>

    </main>
  )
}